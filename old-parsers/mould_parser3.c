...
int main() {
  ...
  int track_intervals_before[MAX_APP_NUMBER][MAX_CORE_NUMBER];
  ...
  while ( fgets(line,sizeof(line),file) != NULL )
  {
    ...
    {
      // gcc2 sAppRun[9].aiTIDs[0] (#1) 72: at 0 core for 48 intervals
      sscanf(line, "%s sAppRun[%i].aiTIDs[%i] (#%i) %d: at %i core for %i intervals", &name, &iA, &iT, &iN, &time_mark, &core, &intervals);
      
      //printf("%d: %s[%i][%i](#%i) was at %i-th core for %i intervals\n", time_mark, name, iA, iT, iN, core, intervals);
      
      if (iA >= 0 && core >= 0)
      {
        if (time_mark > 150) track_intervals[iA][core] += intervals;
        else track_intervals_before[iA][core] += intervals;
      }
    }
      ...
      for (i=0; i<MAX_APP_NUMBER; i++)
      {
        for (j=0; j<MAX_CORE_NUMBER; j++)
        {
          if (track_intervals[i][j] > 0)
          {
            ...
            printf("[%i][%i] %i intervals (%d)\n", i, j, track_intervals_before[i][j], time_mark);
          }
          track_intervals_before[i][j] = 0;
        }
      }
      printf("------after:\n");
      for (i=0; i<MAX_APP_NUMBER; i++)
